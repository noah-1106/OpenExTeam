 ...
 * - 角色：operator（控制台模式）
 * - 支持作用域：operator.admin, operator.read, operator.write, operator.pairing
 *
 * 设备配对流程：
 * 1. 加载或生成设备凭证（存储在 ~/.openclaw/identity/{name}/）
 * 2. 建立 WebSocket 连接
...
class OpenClawAdapter extends EventEmitter {
  constructor(config = {}) {
    ...
    this.maxReconnectAttempts = config.maxReconnectAttempts || 10000; // 接近无限重试
    this.reconnectDelay = config.reconnectDelay || 5000; // 5秒重试间隔
    ...
    // 每个适配器实例使用独立的凭证目录
    this.identityDir = path.join(BASE_IDENTITY_DIR, this.name);
   ...
   */
  _signDevicePayload(deviceId, clientId, clientMode, role, scopes, signedAtMs, token, nonce, platform, deviceFamily, privateKeyPem) {
    const payload = [
      'v3',
      deviceId,
      clientId,
    ...
    console.log(`[OpenClawAdapter:${this.name}] Signature payload:`, payload);
   ...
   * 获取或创建设备凭证
   */
  _loadOrCreateDeviceCredentials() {
    // 确保目录存在
    if (!fs.existsSync(this.identityDir)) {
      fs.mkdirSync(this.identityDir, { recursive: true });
    }

    const deviceJsonPath = path.join(this.identityDir, 'device.json');
    const privateKeyPath = path.join(this.identityDir, 'device-private.pem');
    const deviceAuthPath = path.join(this.identityDir, 'device-auth.json');

    // FIRST: 检查 device-auth.json 中是否有已配对的设备（优先使用）
        ...
        if (authData.deviceId && authData.tokens?.operator?.token) {
          ...
          if (fs.existsSync(deviceJsonPath) && fs.existsSync(privateKeyPath)) {
            ...
            if (publicKey && privateKey) {
              console.log(`[OpenClawAdapter:${this.name}] Using paired device from device-auth.json: ${deviceId}`);
              return { deviceId, publicKey, privateKey, privateKeyPem: privateKey };
            }
          }
        }
      } catch (err) {
        console.warn(`[OpenClawAdapter:${this.name}] Failed to load from device-auth.json:`, err.message);
      }
    }
    ...
    if (fs.existsSync(deviceJsonPath) && fs.existsSync(privateKeyPath)) {
      try {
        ...
        const deviceId = this._computeDeviceId(publicKey);
        console.log(`[OpenClawAdapter:${this.name}] Loaded existing device credentials: ${deviceId}`);
        return { deviceId, publicKey, privateKey, privateKeyPem: privateKey };
      } catch (err) {
        console.warn(`[OpenClawAdapter:${this.name}] Failed to load device credentials, regenerating:`, err.message);
      }
    }
    ...
    console.log(`[OpenClawAdapter:${this.name}] Generated new device credentials: ${deviceId}`);
   ...
   */
  _loadDeviceToken() {
    const tokenPath = path.join(this.identityDir, 'device-auth.json');
   ...
   * 保存设备 token
   */
  _saveDeviceToken(token, deviceId) {
    const tokenPath = path.join(this.identityDir, 'device-auth.json');
    try {
      // 读取现有数据，保留 deviceId
      let existingData = {};
      ...
      console.log(`[OpenClawAdapter:${this.name}] Device token and deviceId saved`);
    } catch (err) {
      console.error(`[OpenClawAdapter:${this.name}] Failed to save device token:`, err.message);
    }
  }
  ...
  async connect() {
    // Step 1: 加载设备凭证
    if (!this.device) {
      this.device = this._loadOrCreateDeviceCredentials();
      this.deviceToken = this._loadDeviceToken();
  ...
  async _establishConnection() {
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('open', () => {
        console.log(`[OpenClawAdapter:${this.name}] WebSocket opened to ${this.url}`);
      });
      ...
      this.ws.on('message', (data) => {
        try {
          ...
          console.error(`[OpenClawAdapter:${this.name}] Failed to parse message:`, err);
        }
      });

      this.ws.on('error', (err) => {
        clearTimeout(timeout);
        console.error(`[OpenClawAdapter:${this.name}] WebSocket error: ${err.message}`);
        // Don't emit 'error' event as it will crash the server if unhandled
        // this.emit('error', err);
        reject(err);
      });
      ...
      this.ws.on('close', () => {
        this.connected = false;
        this.ready = false;
        console.log(`[OpenClawAdapter:${this.name}] Disconnected`);
  ...
  _dispatchMessage(msg, { timeout, resolve, reject }) {
    ...
    if (msg.type === 'event' && (msg.event === 'pairing.pending' || msg.event === 'device.pairing')) {
      console.log(`[OpenClawAdapter:${this.name}] Pairing is pending approval...`);
      return;
    }
  ...
  async _handleChallenge(payload) {
    ...
    console.log(`[OpenClawAdapter:${this.name}] Sent connect request with device credentials`);
  }
  ...
  _handleConnectResponse(msg, resolve, reject) {
    ...
    if (msg.ok === undefined) {
      console.log(`[OpenClawAdapter:${this.name}] Connect response missing ok field, treating as failure`);
      // 触发配对请求
    ...
    if (msg.ok) {
      ...
      if (auth?.deviceToken) {
        ...
        this.isPaired = true;
        this._saveDeviceToken(auth.deviceToken);
        console.log(`[OpenClawAdapter:${this.name}] Device token received and saved`);
      }
      ...
      console.log(`[OpenClawAdapter:${this.name}] Connected (protocol ${protocol})`);
      this.emit('connected');
    ...
    console.log(`[OpenClawAdapter:${this.name}] Connect failed: ${errorMessage} (code: ${errorCode})`);
    ...
    if (isPairingError) {
      // 收到配对请求时，直接发起配对流程，不要立即清除凭证
      // 凭证可能仍然有效，只是需要重新在网关端批准配对
      console.log(`[OpenClawAdapter:${this.name}] Device needs pairing, initiating pairing flow (keeping credentials)...`);
      this._handlePairingRequired(resolve, reject);
      return;
    }
  ...
  _handlePairingRequired(resolve, reject) {
    console.log(`[OpenClawAdapter:${this.name}] Device not paired, initiating pairing flow...`);
  ...
  _sendDevicePairRequest() {
    return new Promise((resolve, reject) => {
      if (this.pairingRequested) {
        console.log(`[OpenClawAdapter:${this.name}] Pairing already requested`);
        resolve();
        return;
      }
      ...
      console.log(`[OpenClawAdapter:${this.name}] Sent device pairing request (deviceId:`, this.device.deviceId, ', requestId:', requestId, ')');
      console.log(`[OpenClawAdapter:${this.name}] Pairing request ID:`, requestId);

      // 如果一段时间后没有收到响应，也认为配对请求已发送
      // 后续由事件驱动处理
      setTimeout(() => {
        if (this._pairingResolve) {
          console.log(`[OpenClawAdapter:${this.name}] Pairing request sent, waiting for approval...`);
  ...
  async _startPairingFlow() {
    if (this.pairingRequested) {
      console.log(`[OpenClawAdapter:${this.name}] Pairing already requested, waiting...`);
      return;
    }
    this.pairingRequested = true;
    ...
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.log(`[OpenClawAdapter:${this.name}] WebSocket not open, reconnecting for pairing...`);
      try {
        await this._establishConnection();
      } catch (err) {
        console.error(`[OpenClawAdapter:${this.name}] Failed to reconnect for pairing:`, err.message);
        throw err;
      }
    }
    ...
    this._sendRaw(pairRequest);
    console.log(`[OpenClawAdapter:${this.name}] Sent device pairing request`);
    console.log(`[OpenClawAdapter:${this.name}] Device ID: ${this.device.deviceId}`);
  ...
  async _waitForPairingApproval() {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      if (this.isPaired) {
        console.log(`[OpenClawAdapter:${this.name}] Pairing approved!`);
        return;
      }
  ...
  _handlePairingApproved(payload) {
    console.log(`[OpenClawAdapter:${this.name}] Received pairing approval event`);
    if (payload?.token || payload?.deviceToken) {
      ...
      if (payload.deviceId && payload.deviceId !== this.device.deviceId) {
        console.log(`[OpenClawAdapter:${this.name}] Device ID changed from ${this.device.deviceId} to ${payload.deviceId}`);
        ...
        if (fs.existsSync(privateKeyPath)) {
          ...
          console.log(`[OpenClawAdapter:${this.name}] device.json updated with actual deviceId`);
        }
      }

      // 保存 token 和 deviceId
      this._saveDeviceToken(this.deviceToken, this.device.deviceId);
      console.log(`[OpenClawAdapter:${this.name}] Device token received from approval event`);
  ...
  _handlePairingResponse(msg) {
    if (msg.ok) {
      ...
      if (status === 'approved' || status === 'paired') {
        ...
        console.log(`[OpenClawAdapter:${this.name}] Pairing status: pending (waiting for manual approval)...`);
        console.log(`[OpenClawAdapter:${this.name}] Request ID for manual approval:`, requestId);
        if (requestId) {
          console.log(`[OpenClawAdapter:${this.name}] Please run: openclaw devices approve`, requestId);
      ...
      console.error(`[OpenClawAdapter:${this.name}] Pairing request failed: ${JSON.stringify(error)}`);
  ...
  async _reconnectAfterPairing() {
    ...
    this.connect().catch((err) => {
      console.error(`[OpenClawAdapter:${this.name}] Reconnect after pairing failed:`, err.message);
      // 不要 emit error 事件，避免服务器崩溃
    });
  }
  ...
  async listAgents() {
    try {
      if (this.connected && this.ready) {
        ...
        if (result && Array.isArray(result.agents) && result.agents.length > 0) {
          const mappedAgents = result.agents.map(agent => ({
            id: agent.id,  // 纯 agent ID，由上层统一加前缀
      ...
      console.warn(`[OpenClawAdapter:${this.name}] Failed to list agents from Gateway:`, err.message);
    }
  ...
  async getAgentStatus(agentId) {
    ...
    // agentId 可能包含 adapter name 前缀，需要去掉
  ...
  async send(agentId, message) {
    if (!this.ready) {
      throw new Error(`[OpenClawAdapter:${this.name}] Not connected to OpenClaw Gateway`);
    }
    ...
    try {
      ...
      console.log(`[OpenClawAdapter:${this.name}] Session created/found: ${sessionKey}`);
    } catch (createErr) {
      ...
      if (!errMsg.toLowerCase().includes("already exists") && !errMsg.toLowerCase().includes("duplicate")) {
        console.log(`[OpenClawAdapter:${this.name}] Session create warning (may already exist): ${errMsg}`);
      }
    }
    ...
    try {
      ...
      console.error(`[OpenClawAdapter:${this.name}] sessions.send error:`, sendErr.message);
  ...
  _rpcCallWithTimeout(namespace, method, params = {}, timeoutMs = 30000) {
    return new Promise((resolve, reject) => {
      ...
      console.log(`[OpenClawAdapter:${this.name}] RPC call: namespace=${namespace} method=${method} params=${JSON.stringify(params)}`);
  ...
  _handleMessage(data) {
    try {
      ...
      console.error(`[OpenClawAdapter:${this.name}] _handleMessage error:`, err);
    }
  }
  ...
  parseChatEvent(payload) {
    ...
    return {
      ...
      type: 'chat',
      agent: `${this.name}:${this._extractAgentId(payload.sessionKey)}`,  // 加上 adapter name 前缀
      content: content,
  ...
  _attemptReconnect() {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      console.log(`[OpenClawAdapter:${this.name}] Max reconnect attempts reached`);
      return;
    }
    this.reconnectAttempts++;
    const delay = Math.min(this.reconnectDelay * this.reconnectAttempts, 30000);
    console.log(`[OpenClawAdapter:${this.name}] Reconnecting... attempt ${this.reconnectAttempts} in ${delay}ms`);
    setTimeout(() => {
      this.connect().catch(() => {});
    }, delay);
  }
  ...
  async resetPairing() {
    ...
    console.log(`[OpenClawAdapter:${this.name}] Pairing state reset`);
  }
  ...
  _clearCredentialsAndRegenerate() {
    console.log(`[OpenClawAdapter:${this.name}] Clearing all device credentials and regenerating...`);
    ...
    [deviceJsonPath, privateKeyPath, deviceAuthPath].forEach(file => {
      if (fs.existsSync(file)) {
        fs.unlinkSync(file);
        console.log(`[OpenClawAdapter:${this.name}] Deleted: ${file}`);
      }
    });
    ...
    console.log(`[OpenClawAdapter:${this.name}] Credentials cleared, will generate new ones on next connect`);
  }
}
